...
const BotLab: React.FC<BotLabProps> = ({ 
  ...
  const handleAIStrategyGenerate = async () => {
      if (!aiStrategyPrompt.trim()) return;
      setIsGeneratingStrategy(true);
      try {
          const result = await generateBotStrategy(aiStrategyPrompt, [manualConfig.symbol]);
          if (result && result.strategy) {
              setManualConfig(prev => ({
                  ...prev,
                  indicator: result.strategy.indicator || prev.indicator,
                  condition: result.strategy.condition || prev.condition,
                  value: result.strategy.value || prev.value,
                  action: result.strategy.action || prev.action,
                  logic: result.strategy.logic || prev.logic,
                  additionalRules: result.strategy.additionalRules || []
              }));
              addNotification?.('Strategy Generated', 'AI successfully parsed your strategy parameters.', 'success');
          } else {
              addNotification?.('Generation Failed', 'Could not parse strategy from prompt.', 'alert');
          }
      } catch (err) {
          addNotification?.('Generation Error', 'An error occurred while generating strategy.', 'alert');
      } finally {
  ...
  const handleToggleAuto = (bot: TradingBot, action: 'autoMode' | 'autoOptimize') => {
      onUpdateBot?.({ ...bot, [action]: !bot[action] });
  };
                      ...
                      <div className="max-w-2xl mx-auto flex flex-col h-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                           <div className="bg-gray-950 border border-gray-800 rounded-2xl p-8 backdrop-blur-sm relative overflow-hidden group">
                              ...
                              <div className="p-4 bg-gray-950 rounded-xl border border-gray-800">
                                  ...
                                  <div className="mb-6 p-3 bg-indigo-950/20 border border-indigo-500/20 rounded-xl">
                                      <label className="text-[10px] text-indigo-400 font-bold uppercase tracking-widest block mb-2">AI Strategy Assistant</label>
                                      <div className="flex gap-2">
                                          <input
                                              type="text"
                                              value={aiStrategyPrompt}
                                              onChange={e => setAiStrategyPrompt(e.target.value)}
                                              placeholder="e.g. 'buy when MACD crosses up 0' or 'sell if RSI > 70'"
                                              className="flex-1 bg-gray-900 border border-gray-800 rounded-lg p-2.5 text-white text-sm focus:border-indigo-500 outline-none transition-all"
                                              onKeyDown={(e) => {
                                                  if (e.key === 'Enter') {
                                                      e.preventDefault();
                                                      handleAIStrategyGenerate();
                                                  }
                                              }}
                                          />
                                          <button 
                                              onClick={handleAIStrategyGenerate}
                                              disabled={!aiStrategyPrompt.trim() || isGeneratingStrategy}
                                              className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white px-4 border border-indigo-500/50 rounded-lg text-xs font-bold transition-all flex items-center gap-2"
                                          >
